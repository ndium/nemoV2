...
class DRLGazebo(Node):
    def __init__(self):
        ...
        ************************************************************"""
        ...
        self.entity_dir_path = os.path.join(os.getenv('DRLNAV_BASE_PATH', '/home/endium/Documents/turtlebot3_drlnav'), 'src', 'turtlebot3_simulations', 'turtlebot3_gazebo', 'models', 'turtlebot3_drl_world', 'goal_box')
        self.entity_path = os.path.join(self.entity_dir_path, 'model.sdf')
        if not os.path.exists(self.entity_path):
            raise FileNotFoundError(f"model.sdf not found at {self.entity_path}")
        ...
        """************************************************************
        ** Initialise ROS publishers, subscribers et clients
        ************************************************************"""
